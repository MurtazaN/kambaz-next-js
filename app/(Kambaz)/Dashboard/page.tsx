...
export default function Dashboard() {
  ...
  const handleEnroll = async (courseId: string) => {
    ...
    try {
      await eClient.enrollUserInCourse((currentUser as any)?._id, courseId);
      dispatch(enrollUserInCourse({ user: (currentUser as any)?._id, course: courseId }));
    } catch (err) {
      console.error(err);
    }
  };
  const handleUnenroll = async (courseId: string) => {
    if (!currentUser) return;
    try {
      await eClient.unenrollUserFromCourse((currentUser as any)?._id, courseId);
      dispatch(
        unenrollUserFromCourse({ user: (currentUser as any)?._id, course: courseId })
      );
    } catch (err) {
      console.error(err);
    }
